...
#include <apriltags_ros/Rect.h>
#include <apriltags_ros/Rects.h>
...
namespace apriltags_ros{
  ...
  AprilTagDetector::AprilTagDetector(ros::NodeHandle& nh, ros::NodeHandle& pnh): it_(nh){
    ...
    proposals_pub_ = nh.advertise<apriltags_ros::Rects>("quad_proposals", 1);
  ...
  void AprilTagDetector::switchCB(const apriltags_ros::BoolStamped::ConstPtr& switch_msg){
  ...
  void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg,const sensor_msgs::CameraInfoConstPtr& cam_info){
    ...
    apriltags_ros::Rects quad_proposal_array;
    ...
    for(int qi = 0; qi < quad_proposals.size(); qi++){
      apriltags_ros::Rect quad_proposal;
